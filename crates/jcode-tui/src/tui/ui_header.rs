...
fn build_header_lines_with_auth(
    ...
    // Credentials are management detail, not permanent chrome. Preserve the
    // remote-host context when attached over SSH, otherwise only surface a
    // concise actionable warning when nothing is configured.
    ...
    } else if configured_auth_count(auth) == 0 {
        lines.push(
            Line::from(vec![
    ...
    if let Some(dir) = app.working_dir() {
        ...
        if let Some(worker) = app.workspace_context_label() {
            text = format!("{text} [worker: {worker}]");
        }
