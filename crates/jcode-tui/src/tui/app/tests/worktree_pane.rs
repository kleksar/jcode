...
#[test]
fn test_empty_composer_arrow_keys_navigate_chat_diff_and_files_with_safe_boundaries() {
    let _lock = scroll_render_test_lock();
    let repo = init_worktree_pane_test_repo();
    crate::tui::ui::prime_worktree_changes_for_tests(repo.path());
    crate::tui::ui::prime_project_tree_for_tests(repo.path());
    ...
    // Empty-chat Right enters the changed-session default, Diff.
    app.handle_key(KeyCode::Right, KeyModifiers::NONE).unwrap();
    assert!(app.diff_pane_focus, "Chat + Right should enter the right workspace");
    assert!(
        !app.worktree_files_tab_active(),
        "a changed session should default to Diff"
    );

    // Diff → Files → Files and Files → Diff → Chat follow the fixed order.
    app.handle_key(KeyCode::Right, KeyModifiers::NONE).unwrap();
    assert!(app.worktree_files_tab_active(), "Diff + Right should enter Files");
    app.handle_key(KeyCode::Right, KeyModifiers::NONE).unwrap();
    assert!(
        app.worktree_files_tab_active() && app.diff_pane_focus,
        "Files + Right is a consumed boundary no-op"
    );
    app.handle_key(KeyCode::Left, KeyModifiers::NONE).unwrap();
    assert!(
        !app.worktree_files_tab_active() && app.diff_pane_focus,
        "Files + Left should return to Diff"
    );
    app.handle_key(KeyCode::Left, KeyModifiers::NONE).unwrap();
    assert!(!app.diff_pane_focus, "Diff + Left should return to Chat");

    let session_id = app.session.id.clone();
    app.handle_key(KeyCode::Left, KeyModifiers::NONE).unwrap();
    assert!(
        app.session_picker_overlay.is_some(),
        "Chat + Left should open Sessions"
    );
    app.handle_key(KeyCode::Right, KeyModifiers::NONE).unwrap();
    assert!(
        app.session_picker_overlay.is_none(),
        "Sessions + Right should dismiss the picker"
    );
    assert_eq!(app.session.id, session_id, "Sessions + Right keeps the session");
}

#[test]
fn test_chat_right_uses_files_without_changes() {
    let _lock = scroll_render_test_lock();
    let repo = tempfile::tempdir().expect("clean project");
    std::process::Command::new("git")
        .current_dir(repo.path())
        .args(["init", "-q"])
        .status()
    ...
    app.handle_key(KeyCode::Right, KeyModifiers::NONE).unwrap();
    assert!(app.diff_pane_focus, "Chat + Right should focus the right workspace");
    assert!(
        app.worktree_files_tab_active(),
        "a clean session should default to Files"
    );
}

#[test]
fn test_nonempty_and_whitespace_composer_input_keep_arrow_cursor_behavior() {
    let mut app = create_test_app();
    app.input = "x".to_string();
    app.cursor_pos = 0;
    app.handle_key(KeyCode::Left, KeyModifiers::NONE).unwrap();
    assert_eq!(app.cursor_pos, 0, "input-start Left must stay in the composer");
    assert!(
        app.session_picker_overlay.is_none(),
        "nonempty input must not open Sessions"
    );
    app.handle_key(KeyCode::Right, KeyModifiers::NONE).unwrap();
    assert_eq!(app.cursor_pos, 1, "Right should retain normal cursor movement");
    assert!(!app.diff_pane_focus, "typed input must not enter the right pane");

    app.input = " ".to_string();
    app.cursor_pos = 0;
    app.handle_key(KeyCode::Right, KeyModifiers::NONE).unwrap();
    assert_eq!(app.cursor_pos, 1, "whitespace is editable composer content");
    assert!(!app.diff_pane_focus, "whitespace must not trigger pane navigation");

    app.input.clear();
    app.cursor_pos = 0;
    app.handle_key(KeyCode::Right, KeyModifiers::CONTROL).unwrap();
    assert!(
        !app.diff_pane_focus,
        "modified arrows must bypass workspace navigation"
    );
}
