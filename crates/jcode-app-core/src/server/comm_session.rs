...

    {
        ...
        members.insert(
            ...
            SwarmMember {
                ...
                todo_progress: None,
                todo_items: Vec::new(),
                // The headed client's attach path fills runtime metadata from
                // its initialized provider. Do not show the requested spawn
                // effort here: provider normalization or unsupported efforts
                // can make it differ from the effective value.
...
pub(super) async fn handle_comm_single_agent(
    ...
    if requesting_session_id != target_root_session_id {
        let _ = client_event_tx.send(ServerEvent::Error {
            id,
            message:
                "Single-agent override requires the requesting session to be the target swarm root."
                    .to_string(),
            retry_after_secs: None,
        });
        return;
    }
    let is_root = swarm_members
        .read()
        .await
    ...
    if !is_root {
        let _ = client_event_tx.send(ServerEvent::Error {
            id,
            message: "Single-agent override is available only to the current swarm root."
                .to_string(),
            retry_after_secs: None,
        });
        return;
